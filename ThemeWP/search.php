...
<section class="best-contents">
		<?php if ( have_posts() ) { ?>
			<article class="best-search-title">
				<h2 class="best-title">
					<?php printf( __( 'Результати пошуку', 'best' ) . ': %s', '<span>' . get_search_query() . '</span>' ); ?>
				</h2>
			...
			<?php while ( have_posts() ) {
				...
				<section id="best-post-<?php the_ID(); ?>" <?php post_class( 'best-post' ); ?>>
					<header class="page-header">
						<h2 class="best-title">
							<a href="<?php the_permalink(); ?>" rel="bookmark" title="<?php the_title_attribute( array( 'before' => __( 'Посилання до ', 'best' ), 'after' => '' ) ); ?>"><?php the_title(); ?></a>
						</h2>
						<p class="best-p-post-data">
							<span class="post-data"><?php _e( 'запостовано', 'best' ); ?> </span>
							<a href="<?php the_permalink(); ?>">
								<?php echo get_the_date(); ?>
							</a>
							<span class="post-data"> <?php _e( 'від', 'best' ); ?> </span>
							<?php the_author_posts_link();
							if ( has_category() ) { ?>
								<span class="post-data"> <?php _e( 'в', 'best' ); ?> </span>
								<?php the_category( ', ' );
							}
							edit_post_link( __( ' Редагувати', 'best' ), '<span class="best-edit-link">', '</span>' ); ?>
						</p>
					</header> <!-- .page-header -->
			...
			<nav class="best-link">
				<div class="best-previous-posts-link">
					<?php next_posts_link( __( 'Попередній пости', 'best' ) . ' ' ); ?>
				</div> <!-- best-previous-posts-link -->
				<div class="best-next-posts-link">
					<?php previous_posts_link( ' ' . __( 'Наступні пости', 'best' ) ); ?>
				</div> <!-- best-next-posts-link -->
			</nav> <!-- .best-link -->
			<div class="best-clear"></div>
		<?php } else { ?>
			<h2 class="best-title">
				<?php printf( __( 'Поруковий результат', 'best' ) . ': %s', '<span>' . get_search_query() . '</span>' ); ?>
			</h2>
			<p><?php _e( 'Вибачте нічого не знайдено, попробуйте щераз', 'best' ); ?></p>
			<?php get_search_form(); ?>
			<div class="best-clear"></div>
		<?php } // if have_posts() ?>
